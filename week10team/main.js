...
function getQuakesForLocation() {
  ...
  geoURL = baseURL + `&latitude=${location.latitude}&longitude=${location.longitude}&maxradiuskm=100`;
  console.log(geoURL);
  return geoURL;
})
}
...
async function showQuakes() {
  ...
  const query = await getQuakesForLocation();
  const quakes = getJSON(query)

...
class CommentModel {
  constructor(type) {
    this.type = type;
    // ge the initial list of comments out of localStorage if it exists
    this.comments = readFromLS(this.type) || [];
  }

  // decided to combime getAllComments, and filterCommentsByName methods into one by passing in an optional query argument
  getComments(q = null) {
    if (q === null) {
...
function readFromLS(key) {
  // The string we retrieve from loaclStorage needs to be converted back to an object with JSON.parae.
  return JSON.parse(window.localStorage.getItem(key));
}

//These methods create the HTML that is needed to output our list of comments to the screen.
// Anything dealing with the output to the browser is catagorized as view code.
const commentIU = `<div class="addComment">
<h2>Add a Comment</h2>
...
function renderCommentList(element, comments) {
  // clear any comments that might be listed
  element.innerHTML = '';
...
// Comments: this code handles getting the list of comments from the data source, and outputting them
// to the screen at the right time. This is often catagorized as Controller code.
...
class Comments {
  ...
  addSubmittListener(postName) {
    // use element.ontouched to avoid more than one listener getting attached at a time to the button.
